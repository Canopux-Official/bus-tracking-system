        ...
        <div className="sm-scroll">
          <div className="sm-timeline">
            {route.map((stop, idx) => {
              ...
              // ✅ Fix: line is active for boarding stop and all between stops (up to but not including drop)
              const lineActive = idx >= boardingIdx && idx < dropIdx && boardingIdx !== -1 && dropIdx !== -1;
              ...
              return (
                <div key={idx} className={`sm-stop sm-stop--${type}`}>
                  <div className="sm-track">
                    ...
                    {!isLast && (
                      <div className={`sm-line ${lineActive ? "sm-line--active" : ""}`} />
                    )}
                  </div>
